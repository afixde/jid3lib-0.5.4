...
/**
 * Link to an image files (BMP or JPG format). Image lines include filename,
 * description and timestamp separated by delimiter - two ASCII chars 124
 * ("||"). Description and timestamp are optional, but if timestamp is used, and
 * there is no description, two delimiters ("||||") should be used between the
 * filename and the timestamp. Multiple images are allowed by using a [CR][LF]
 * delimiter between each image line. No [CR][LF] is needed after the last image
 * line. Number of images is not limited (except by the field size).<BR>
 * <B>Filename</B> can be in one of these formats:
 * <UL>
 * <LI>Filename only - when the image is located in the same path as the MP3
 * file (preferred, since if you move the mp3 file this will still be correct)
 * <LI>Relative Path + Filename - when the image is located in a subdirectory
 * below the MP3 file (i.e. images\cover.jpg)
 * <LI>Full path + Filename - when the image is located in a totally different
 * path or drive. This will not work if the image is moved or drive letters has
 * changed, and so should be avoided if possible (i.e. c:\images\artist.jpg)
 * </LI>
 * </UL>
 * <B>Description</B> can be up to 250 chars long.<BR>
 * <B>Timestamp</B> must be formatted like the lyrics timestamp which is
 * "[mm:ss]". If an image has a timestamp, then the visible image will
 * automatically switch to that image on the timestamp play time, just the same
 * as the selected lyrics line is switched based on timestamps.
 *
 * @author Eric Farng
