...
/**
 * This composite subclass is the leaf of the tree. It contains the actual
 * strings found in the filename.
 *
 * @author Eric Farng
...
public class FilenameToken extends AbstractFilenameComposite {
	...
	public FilenameToken(final FilenameToken copyObject) {
		...
		try {
			...
			throw new NullPointerException(
					"IllegalAccessException: No access to run constructor to create copy " + ex.getMessage());
		} catch (InstantiationException ex) {
			throw new NullPointerException(
					"InstantiationException: Unable to instantiate constructor to copy " + ex.getMessage());
		}
	 ...
	 * <code>FilenameToken</code> matches the keyword, the token's class is set.
	 *
	 * @param matchId3v2FrameBodyClass
	 *            Class of keywords to match against.
	 */
		...
		if (AbstractID3v2FrameBody.class.isAssignableFrom(matchId3v2FrameBodyClass)) {
			if (TagOptionSingleton.getInstance().isCompositeMatchOverwrite() || id3v2FrameBodyClass == null) {
				...
				while (iterator.hasNext()) {
					...
					if (matchString.equals(lowerCaseToken) || matchString.indexOf(lowerCaseToken) >= 0
	...
	public void matchAgainstTag(final AbstractMP3Tag matchTag) {
		if (TagOptionSingleton.getInstance().isCompositeMatchOverwrite() || id3v2FrameBodyClass == null) {
			...
			while (iterator.hasNext()) {
				...
				if (lowerCaseToken.equals(matchString) || matchString != null
