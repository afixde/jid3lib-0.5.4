...
public class FilenameTagBuilder {
    ...
    private static String[] parseDelimiter(final String token) {
        ...
        if (token != null && token.length() > 0) {
            final Iterator<String> iterator = TagOptionSingleton.getInstance().getFilenameDelimiterIterator();
            int index;
            String delimiter;
            while (iterator.hasNext()) {
    ...
    private static String[] parseEndWordDelimiter(final String token) {
        String[] tokenArray = null;
        if (token != null && token.length() > 0) {
            final Iterator<String> iterator = TagOptionSingleton.getInstance().getEndWordDelimiterIterator();
            int index;
            String delimiter;
            while (iterator.hasNext()) {
                delimiter = iterator.next();
    ...
    private static String[] parseParenthesis(final String token) throws TagException {
        ...
        if (token != null && token.length() > 0) {
            ...
            final Iterator<String> iterator = option.getOpenParenthesisIterator();

            // find first parenthesis
            while (iterator.hasNext()) {
                tempOpen = iterator.next();
    ...
    private static String[] parseStartWordDelimiter(final String token) {
        String[] tokenArray = null;
        if (token != null && token.length() > 0) {
            final Iterator<String> iterator = TagOptionSingleton.getInstance().getStartWordDelimiterIterator();
            int index;
            String delimiter;
            while (iterator.hasNext()) {
                delimiter = iterator.next();
